...
void dddWindow::changeSpeed (int axis1, int axis2)
{
  double tmp;
...
void dddWindow::rotate (int axis1, int axis2)
{
  double tmp;
  int i;
  ...
  ( (dddMainWidget*) centralWidget () ) -> multMatrix ();
  tmp = M (0, 1); M (0, 1) = M (1, 0); M (1, 0) = tmp;
  tmp = M (0, 2); M (0, 2) = M (2, 0); M (2, 0) = tmp;
  tmp = M (2, 1); M (2, 1) = M (1, 2); M (1, 2) = tmp;
...
void dddWindow::timerEvent (QTimerEvent *te)
{
